...
export function generateDemoStreams(): DemoStream[] {
  // Generate random participant count (15-50) and duration (30-180 minutes)
  const randomParticipants = () => Math.floor(Math.random() * 36) + 15;
  const randomDuration = () => Math.floor(Math.random() * 151) + 30;
  ...
  return [
    {
      ...
      participantCount: randomParticipants(),
